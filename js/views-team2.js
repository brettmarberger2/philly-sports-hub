...
async function tabLegends(t, { mount }) {
  ...
  mount.innerHTML = `
    <div class="section" style="margin-top:0"><h2>Retired numbers</h2><div class="retired">${CUR.retired[t.key].map(r => `<div class="num-b"><div class="n">${esc(r.n)}</div><div style="font-weight:700">${esc(r.name)}</div><div class="small muted">${esc(r.years)}</div></div>`).join('')}</div></div>
    <div class="section"><h2>Franchise icons</h2><div class="grid g2">${CUR.legends[t.key].map(l => `<div class="legend-card"><div class="n">${l.n ? '#' + esc(l.n) : '★'}</div><div><b>${esc(l.name)}</b> <span class="badge">${esc(l.pos)}</span> <span class="dim small">${esc(l.era)}</span><div class="muted small" style="margin-top:4px">${esc(l.bio)}</div></div></div>`).join('')}</div></div>
    <div class="section"><h2>Defining moments</h2><div class="stack">${moments.map(([y, n]) => { const s = H.find(t.key, y); return `<a class="card flat" style="color:inherit;display:block" href="#/season/${t.key}/${y}"><div class="row between"><b>${esc(n.t)}</b><span class="badge ${s?.result === 'champion' ? 'gold' : ''}">${esc(s ? H.label(t, s) : y)}${s?.result === 'champion' ? ' 🏆' : ''}</span></div><div class="muted small" style="margin-top:4px">${esc(n.s)}</div></a>`; }).join('')}</div></div>
